#!/bin/bash
# Launch nut_v5 PEFM + EquiBot side-by-side in tmux session "pefm_v5".
...
PEFM_CMD="source ~/miniconda3/etc/profile.d/conda.sh && conda activate pefm \
  ...
  && python3 -u -m pefm.train \
    ...
    wandb.project=pefm \
  2>&1 | tee $LOG_DIR/nut_pefm_v5.log"
...
EQUIBOT_CMD="source ~/miniconda3/etc/profile.d/conda.sh && conda activate pefm \
  ...
  2>&1 | tee $LOG_DIR/nut_equibot_v5.log"
...
echo "  pefm log:    $LOG_DIR/nut_pefm_v5.log"
echo "  equibot log: $LOG_DIR/nut_equibot_v5.log"
echo "  wandb:   pefm/nut_pefm_v5  +  equibot/nut_equibot_v5"
