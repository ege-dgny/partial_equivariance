...
from .utils.transformations import quat_multiply, axisangle2quat, quat2mat
from .utils.frame_converter import wrap_angle
from .utils.convert_coords import (
    global_to_local_pose,
    local_to_global_pose,
)
from .utils.bullet_robot import BulletRobot
from .utils.info import SIM_ROBOT_INFO, DEFAULT_ANCHOR_CONFIG
from .utils.render import add_debug_region
from .utils.plan_control_traj import plan_control_traj
from .utils.anchors import get_closest
from .utils.init_utils import (
    load_rigid_object,
    load_soft_object,
    rotate_around_z,
)
from .utils.multi_camera import MultiCamera
...
class BaseEnv(object):
    ...
    def step(self, action, dummy_reward=False, dummy_obs=False):
        ...
        # sleep (only when visualizing, skip for headless demo generation)
    ...
    def _sample_primitive_surface(self, obj_id, num_samples=200):
        ...
        if shape_type == pybullet.GEOM_CYLINDER:
            ...
            n = num_samples
            # sample points on cylinder surface and caps
            angles = np.random.uniform(0, 2 * np.pi, n)
            # 70% side, 15% top cap, 15% bottom cap
            n_side = int(n * 0.7)
            n_cap = (n - n_side) // 2
            # side
            ...
            # bottom cap
            r_bot = np.sqrt(np.random.uniform(0, 1, n - n_side - n_cap)) * radius
            pts_bot = np.stack([
                r_bot * np.cos(angles[n_side + n_cap:]),
                r_bot * np.sin(angles[n_side + n_cap:]),
                np.full(n - n_side - n_cap, -height / 2),
            ], axis=-1)
