...
from pefm.utils.media import combine_videos, save_video
    ...
    for ep_ix in range(num_episodes):
        ...
        while not done:
            ...
            if log_dir is not None:
                ...
                # Phase 1.4: capture per-step selector metadata. In single-env
                # eval ac_dict is a single dict (not a list) — see agent.act().
                if (
                    ac_dict is not None
                    ...
                    history["selector_params"].append(
                        np.asarray(ac_dict["selector_params"])
                    )
                else:
        ...
        if log_dir is not None:
            ...
            # Reconcile selector_params None placeholders into a NaN-padded array.
...
def main(cfg):
    ...
    if cfg.env.vectorize:
        ...
        from pefm.vec_eval import run_eval as run_vec_eval
